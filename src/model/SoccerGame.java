...
public class SoccerGame {
	...
	public void automateGoalkeeper() {
		...
		Striker striker = (Striker) gamePlayers.get("Striker");
		if (soccerBall.onGoalkeeperSide()) {
			goalkeeper.grabsBall();
			if(!soccerBall.inGate()) {
				goalkeeper.setPlayerStatistics(goalkeeper.getPlayerStatistics() + 1);
				//System.out.println("Balls caught updated to: " + goalkeeper.getPlayerStatistics());
			}
			else{
				//System.out.println("From Striker");
				//striker.setPlayerStatistics(striker.getPlayerStatistics() + 1);
				//System.out.println("Goals updated to: " + striker.getPlayerStatistics());
			}
...
//			System.out.println("Moving");
	...
	public boolean isScored() {
		Goalkeeper goalkeeper = (Goalkeeper) gamePlayers.get("Goalkeeper");
		return !goalkeeper.isPlayerHasBall() && SoccerBall.getSoccerBall().inGate();
	}
